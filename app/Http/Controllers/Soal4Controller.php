    ...
    public function update(Request $request)
    {
        ...
        foreach ($validatedData as $field => $file) {
            if ($request->hasFile($field)) {
                if (!empty($soal4->$field)) {
                    Storage::disk('public')->delete($soal4->$field);
                }
                $soal4->$field = $file->store('uploads/pdf', 'public');
            }
        }

        // Perhitungan ulang nilai
        $nilai = 0;
        if (!empty($soal4->independent_org)) $nilai += 8;
        if (!empty($soal4->foreign_school_degree)) $nilai += 7;
        ...
        return response()->json(['message' => 'Berhasil mengupdate data!', 'data' => $soal4]);
    }
